extends Node3D
## This file is part of the program responsible for the 3D physics test
##
...
var source: PackedScene = preload("res://src/3D/ball.tscn")

@onready var from: Node3D = $From
...
func _on_create_object() -> void:
	for index: int in iterator:
		var node: Node3D = source.instantiate()
		node.position.x = from_x + (index * 0.5)
		add_child(node)
...
func _ready() -> void:
	info.set_physics(ProjectSettings.get_setting_with_override("physics/3d/physics_engine"))
